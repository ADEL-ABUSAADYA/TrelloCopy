...
{
    public record ResetPasswordCommand(string otp  , string email , string NewPassword, string ConfirmPassword) : IRequest<RequestResult<bool>>;


    public class ResetPasswordCommandHandeler : UserBaseRequestHandler<ResetPasswordCommand, RequestResult<bool>>
    {
        public ResetPasswordCommandHandeler(UserBaseRequestHandlerParameters parameters) : base(parameters)
        {
        }

        public override async Task<RequestResult<bool>> Handle(ResetPasswordCommand request, CancellationToken cancellationToken)
        {
           var userExist = await  _userRepository.Get(u=> u.Email == request.email && u.ResetPassword == request.otp).FirstOrDefaultAsync();
